...
std::vector<Star> createStars(uint32_t count, float scale)
{
    ...
    // Create random distributed stars on screen
    for (uint32_t i{ count }; i--;)
    {
        float const x = (dis(gen) - 0.5) * conf::window_size_f.x * scale;
        float const y = (dis(gen) - 0.5) * conf::window_size_f.y * scale;
        float const z = dis(gen) * (conf::far - conf::near) + conf::near;

        // discard any stars that fall into the zone
...
void updateGeometry(uint32_t idx, Star const& s, sf::VertexArray& va)
{
    ...
    va[i + 0].position = { p.x - r, p.y - r }; // top left
    va[i + 1].position = { p.x + r, p.y - r }; // top right
    va[i + 2].position = { p.x - r, p.y + r }; // bottom left
    va[i + 3].position = { p.x - r, p.y + r }; // bottom left
    va[i + 4].position = { p.x + r, p.y - r }; // top right
    va[i + 5].position = { p.x + r, p.y + r }; // bottom right
...
int main()
{
    auto window = sf::RenderWindow(sf::VideoMode({ conf::window_size.x, conf::window_size.y }), "Star Travel Project", sf::Style::None);
    ...
    for (int32_t idx{ conf::count }; idx--;)
    {
        uint32_t const i = 6 * idx;
        va[i + 0].texCoords = { 0.0f, 0.0f };                         // top left
        va[i + 1].texCoords = { texture_size_f.x, 0.0f };             // top right
        va[i + 2].texCoords = { 0.0f, texture_size_f.y };             // bottom left
        va[i + 3].texCoords = { 0.0f, texture_size_f.y };             // bottom left
        va[i + 4].texCoords = { texture_size_f.x, 0.0f };             // top right
        va[i + 5].texCoords = { texture_size_f.x, texture_size_f.y }; //bottom right
    }
    ...
    while (window.isOpen())
    {
        ...
        for (uint32_t i{conf::count}; i--;)
        {
            ...
            if (s.z < conf::near)
            {
                ...
                // this star is now the first we need to draw, because it is the furthest away from us
                first = i;
            }
        }
